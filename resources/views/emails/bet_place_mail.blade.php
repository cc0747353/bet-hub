    ...
    @php
        $color = 'primary';
                $google_map  = '<a target="_blank" class="button button-primary" style="margin-bottom: 15px"  href="'.route('user.bets-details').'">View</a>';
                    $search = array('#currency#','#amount#','#option#','#match#','#question#','#return_amount#','#post_balance#','#link#');
                    $replace = array($currency,$amount,$option,$match,$question,$return_amount,$post_balance,$google_map);
