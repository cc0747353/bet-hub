<div>
    ...
    <div class="row mb-5">
        ...
        <div class="col-6">
            ...
            {{ Form::text('user_name', $user->user_name, ['class' => 'form-control form-control-solid', 'placeholder' => __('messages.user.user_name'), 'required']) }}
        </div>
    </div>
    <div class="row mb-5">
        <div class="col-6">
            ...
            <div class="position-relative">
                {{Form::password('password',['class' => 'form-control userPassword','placeholder' => __('messages.user.password'),'autocomplete' => 'off','aria-label'=>"Password",'data-toggle'=>"password"])}}
        ...
        <div class="col-6">
            {{ Form::label('Confirm Password',__('messages.user.password_confirmation').':' ,['class' => 'form-label']) }}
            <span data-bs-toggle="tooltip" title="{{ __('messages.flash.user_8_or') }}">
                <i class="fa fa-question-circle"></i>
            </span>
            <div class="position-relative">
                {{Form::password('password_confirmation',['class' => 'form-control','placeholder' => __('messages.user.password_confirmation'),'autocomplete' => 'off','aria-label'=>"Password",'data-toggle'=>"password"])}}
                <span class="position-absolute d-flex align-items-center top-0 bottom-0 end-0 me-4 input-icon input-password-hide cursor-pointer text-gray-600"><i class="bi bi-eye-slash-fill"></i></span>
            </div>
        </div>
    </div>
    <div class="row mb-5">
        <div class="col-6">
            <div class="form-group">
                ...
                {{ Form::select('role',getAllRole(), $user->role_display_id, ['class' => 'form-control','io-select2' ,'data-control'=>'select2']) }}
            </div>
        </div>
        <div class="col-6">
                ...
                <div class="mb-3" io-image-input="true">
                    ...
                    <div class="d-block">
                        <div class="image-picker">
                            @php
                                $imageCss = 'style="background-image: url('.(!empty($user->profile_image) ? $user->profile_image : asset('images/avatar.png')).')"';
                            @endphp
    ...
    <div class="row mb-5">
        <div class="col-6">
            ...
            {{ Form::text('address_1',isset($address) ? $address['address_1'] : null , ['class' => 'form-control form-control-solid', 'placeholder' => __('messages.user.address_1')]) }}
        </div>
        <div class="col-6">
            {{ Form::label('address_2', __('messages.user.address_1').':', ['class' => 'form-label']) }}
            {{ Form::text('address_2', isset($address) ? $address['address_2'] : null, ['class' => 'form-control form-control-solid', 'placeholder' => __('messages.user.address_2')]) }}
        </div>
    </div>
    <div class="row mb-5">
        <div class="col-6">
            {{ Form::label('country_id',__('messages.user.country').':' ,['class' => 'form-label required']) }}
            {{ Form::select('country_id',  getCountries(), isset($address) ? $address['country_id']:null,['class' => 'io-select2 form-select', 'data-control'=>"select2",'placeholder' => __('Select country'), 'required', 'id'=>'countryId']) }}
        </div>
        <div class="col-6">
            {{ Form::label('state', __('messages.user.state').':', ['class' => 'form-label']) }}
            {{ Form::select('state', getStates(), isset($address) ? $address['state'] : null, ['class' => 'io-select2 form-select', 'data-control'=>"select2", 'placeholder' => __('messages.common.select_state'), 'id' => 'stateId']) }}
        </div>
    </div>
    <div class="row mb-5">
        <div class="col-6">
            {{ Form::label('city', __('messages.user.city').':', ['class' => 'form-label']) }}
            {{ Form::select('city', getCities(), isset($address) ? $address['city'] : null, ['class' => 'io-select2 form-select', 'data-control'=>"select2", 'placeholder' => __('messages.common.select_city'), 'id' => 'cityId']) }}
        </div>
        <div class="col-6">
            {{ Form::label('zip', __('messages.user.zip').':', ['class' => 'form-label']) }}
            {{ Form::text('zip', isset($address) ? $address['zip'] : null, ['class' => 'form-control', 'placeholder' => __('messages.user.zip'), 'onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")']) }}
        </div>
    </div>
</div>
<div class="d-flex">
