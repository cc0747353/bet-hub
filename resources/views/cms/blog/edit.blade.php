                            ...
                            <div class="row">
                                <div class="col-sm-8">
                                    <div class="col-sm-12 mb-5">
                                        ...
                                        {{ Form::text('title', $blog['title'] ?? null, ['id'=>'blogTitle','class' => 'form-control', 'placeholder' => __('messages.blog.title')]) }}
                                    </div>
                                    <div class="col-sm-12 mb-5">
                                        {{ Form::label('slug',__('messages.blog.slug').' :', ['class' => 'form-label required']) }}
                                        {{ Form::text('slug', isset($blog) ? $blog->slug : null, ['class' => 'form-control','required', 'id'=>'blogSlug', 'placeholder' => __('messages.blog.slug')]) }}
                                        <input type="hidden" name="slug" id="slugHidden"  value="{{  isset($blog) ? $blog->slug : null }}">
                                    </div>
                                    <div class="col-sm-12 mb-5">
                                        {{ Form::label('tag',__('Tag').(':'), ['class' => 'form-label']) }}
                                        <span class="required"></span>
                                        {{ Form::text('tag', $blog['tag'] ?? null, ['id'=>'editBlogTag','class' => 'form-control', 'placeholder' => __('messages.blog.tag')]) }}
                                    </div>
                                </div>
                                <div class="col-sm-4">
                                    ...
                                    <div class="mb-3" io-image-input="true">
                                        ...
                                        <div class="d-block">
                                            <div class="image-picker">
                                                @php
                                                    $imageCss = 'style="background-image: url('.(!empty($blog['blog_image']) ? $blog['blog_image'] : asset('images/avatar.png')).')"';
                                                @endphp
                                                <div class="image previewImage blogImage" id="exampleInputImage"{!! $imageCss !!}></div>
