                    ...
                    <div class="col-xxl-8 col-lg-6">
                        ...
                        <div class="tabs-section">
                            <ul class="nav nav-tabs mb-20" id="myTab" role="tablist">
                                @foreach($leagueData as $data)
                                    <li class="nav-item" role="presentation">
                                        <button class="nav-link me-1 {{ $league_id == $data->id ? 'active' : '' }}" id="{{ $data->category->name }}-tab" data-bs-toggle="tab"
                                                data-bs-target="#{{ $data->category->name }}" type="button" role="tab"
                                                aria-controls="{{ $data->category->name }}" aria-selected="true">
                                            <i class="{{ $data->category->icon }} fs-5 me-3"></i>
                                            <span class="fs-6">{{ $data->category->name }}</span>
                                        </button>
                                    </li>
                                @endforeach
                            </ul>
                            <div class="tab-content" id="myTabContent">
                                @foreach($leagueData as $data)
                                    <div class="tab-pane fade show {{ $league_id == $data->id ? 'active' : '' }}" id="{{ $data->category->name }}" role="tabpanel"
                                         aria-labelledby="{{ $data->category->name }}-tab">
