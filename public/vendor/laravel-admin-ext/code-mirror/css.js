...
(function (mod) {
    ...
    CodeMirror.defineMode('css', function (config, parserConfig) {
        ...
        if (!parserConfig.propertyKeywords) parserConfig = CodeMirror.resolveMode(
            'text/css')
        ...
        function tokenBase (stream, state) {
            ...
            if (ch == '@') {
                ...
                return ret(null, ch)
                    ...
                    stream.match(/rl(-prefix)?\(/i)) ||
                ((ch == 'd' || ch == 'D') &&
                    stream.match('omain(', true, true)) ||
                ((ch == 'r' || ch == 'R') &&
                    stream.match('egexp(', true, true))) {
                stream.backUp(1)
        ...
        function pushContext (state, stream, type, indent) {
            state.context = new Context(type,
        ...
        states.top = function (type, stream, state) {
            ...
            } else if (/^@(media|supports|(-moz-)?document|import)$/i.test(
                type)) {
        ...
        states.prop = function (type, stream, state) {
            ...
            if (type == '{' && allowNested) return pushContext(state, stream,
                'propBlock')
            if (type == '}' || type == '{') return popAndPass(type, stream,
                state)
        ...
        states.parens = function (type, stream, state) {
            ...
            if (type == ')') return popContext(state)
            if (type == '(') return pushContext(state, stream, 'parens')
            if (type == 'interpolation') return pushContext(state, stream,
                'interpolation')
        ...
        return {
            startState: function (base) {
                return {
                    ...
                    stateArg: null,
                    context: new Context(inline ? 'block' : 'top', base || 0,
                        null),
                }
            },
    ...
    var allWords = documentTypes_.concat(mediaTypes_).
