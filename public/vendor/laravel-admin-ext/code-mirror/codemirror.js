...
// detected are enabled based on userAgent etc sniffing.
    ...
    var ie_version = ie &&
        (ie_upto10 ? document.documentMode || 6 : +(edge || ie_11up)[1])
...
// Some browsers use the wrong event properties to signal cmd/ctrl on OS X
    ...
    var rmClass = function (node, cls) {
        ...
        if (match) {
            ...
            node.className = current.slice(0, match.index) +
                (after ? match[1] + after : '')
        }
    }

    function removeChildren (e) {
        for (var count = e.childNodes.length; count >
        0; --count) { e.removeChild(e.firstChild) }
        return e
    }
...
// wrapper for elt, which removes the elt from the accessibility tree
    ...
    function activeElt () {
        ...
        activeElement.shadowRoot.activeElement) { activeElement = activeElement.shadowRoot.activeElement }
...
// a particular column.
    function findColumn (string, goal, tabSize) {
        for (var pos = 0, col = 0; ;) {
            ...
            if (nextTab == string.length || col + skipped >=
                goal) { return pos + Math.min(skipped, goal - col) }
            col += nextTab - pos
...
// display-related state.
    ...
    function Display (place, doc, input) {
        ...
        d.selectionDiv = elt('div', null, null,
        ...
        d.heightForcer = elt('div', null, null,
        ...
        if (old) {
            for (var i = 0; i < old.length; ++i) {
                ...
                var startsBefore = span.from == null ||
                    (marker.inclusiveLeft ? span.from <= startCh : span.from <
                        startCh)
                if (startsBefore || span.from == startCh && marker.type ==
                    'bookmark' && (!isInsert || !span.marker.insertLeft)) {
                    var endsAfter = span.to == null ||
                        (marker.inclusiveRight ? span.to >= startCh : span.to >
                            startCh)
                    ;(nw || (nw = [])).push(new MarkedSpan(marker, span.from,
        ...
        if (old) {
            for (var i = 0; i < old.length; ++i) {
                ...
                if (endsAfter || span.from == endCh && marker.type ==
                    'bookmark' && (!isInsert || span.marker.insertLeft)) {
                    var startsBefore = span.from == null ||
                        (marker.inclusiveLeft ? span.from <= endCh : span.from <
                            endCh)
                    ;(nw || (nw = [])).push(new MarkedSpan(marker,
...
// arrays with one element for each line in (after) the change.
    function stretchSpansOverChange (doc, change) {
        ...
        var oldFirst = isLine(doc, change.from.line) &&
            getLine(doc, change.from.line).markedSpans
        var oldLast = isLine(doc, change.to.line) &&
        ...
        if (first) {
            ...
            for (var i = 0; i < first.length; ++i) {
                ...
                if (span.to == null) {
                    ...
                    if (!found) { span.to = startCh } else if (sameLine) {
    ...
    function clearEmptySpans (spans) {
        for (var i = 0; i < spans.length; ++i) {
            ...
            if (span.from != null && span.from == span.to &&
                span.marker.clearWhenEmpty !== false) { spans.splice(i--, 1) }
        }
    ...
    function removeReadOnlyRanges (doc, from, to) {
        ...
        for (var i = 0; i < markers.length; ++i) {
            ...
            for (var j = 0; j < parts.length; ++j) {
                ...
                var newParts = [j, 1], dfrom = cmp(p.from, m.from),
                    dto = cmp(p.to, m.to)
        ...
        if (sps) {
            for (var sp = (void 0), i = 0; i < sps.length; ++i) {
                ...
                if (sp.marker.collapsed && (start ? sp.from : sp.to) == null &&
                    (!found || compareCollapsedMarkers(found, sp.marker) <
                        0)) { found = sp.marker }
            }
        }
        return found
    ...
    function collapsedSpanAround (line, ch) {
        ...
        if (sps) {
            for (var i = 0; i < sps.length; ++i) {
                var sp = sps[i]
                if (sp.marker.collapsed && (sp.from == null || sp.from < ch) &&
                    (sp.to == null || sp.to > ch) &&
                    (!found || compareCollapsedMarkers(found, sp.marker) <
                        0)) { found = sp.marker }
            }
        ...
        if (sps) {
            for (var i = 0; i < sps.length; ++i) {
                ...
                var toCmp = cmp(found.to, to) || extraRight(sp.marker) -
                    extraRight(marker)
                if (fromCmp >= 0 && toCmp <= 0 || fromCmp <= 0 && toCmp >=
                    0) { continue }
                if (fromCmp <= 0 &&
                    (sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp(
                        found.to, from) >= 0 : cmp(found.to, from) > 0) ||
                    fromCmp >= 0 &&
                    (sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp(
                        found.from, to) <= 0 : cmp(found.from, to) <
                        0)) { return true }
            }
        }
    }
...
// they are entirely covered by collapsed, non-widget span.
    ...
    function lineIsHiddenInner (doc, line, span) {
        ...
        if (span.marker.inclusiveRight && span.to ==
            line.text.length) { return true }
...
// BIDI HELPERS
    ...
    function iterateBidiSections (order, from, to, f) {
        ...
        for (var i = 0; i < order.length; ++i) {
            var part = order[i]
            if (part.from < to && part.to > from || from == to && part.to ==
                from) {
                f(Math.max(part.from, from), Math.min(part.to, to),
                    part.level == 1 ? 'rtl' : 'ltr', i)
                found = true
            }
        }
    ...
    function getBidiPartAt (order, ch, sticky) {
        ...
        for (var i = 0; i < order.length; ++i) {
            ...
            if (cur.to == ch) {
                if (cur.from != cur.to && sticky ==
                    'before') { found = i } else { bidiOther = i }
            }
            if (cur.from == ch) {
                if (cur.from != cur.to && sticky !=
                    'before') { found = i } else { bidiOther = i }
...
// objects) in the order in which they occur visually.
    var bidiOrdering = (function () {
        ...
        function charType (code) {
            if (code <= 0xf7) { return lowTypes.charAt(code) } else if (0x590 <=
                code && code <= 0x5f4) { return 'R' } else if (0x600 <= code &&
                code <= 0x6f9) {
                return arabicTypes.charAt(code - 0x600)
            } else if (0x6ee <= code && code <=
                0x8ac) { return 'r' } else if (0x2000 <= code && code <=
                0x200b) { return 'w' } else if (code ==
                0x200c) { return 'b' } else { return 'L' }
        }
        ...
        return function (str, direction) {
            ...
            for (var i$2 = 0, cur = outerType; i$2 < len; ++i$2) {
                ...
                if (type$1 == '1' && cur ==
                    'r') { types[i$2] = 'n' } else if (isStrong.test(type$1)) {
                    cur = type$1
            ...
            for (var i$3 = 1, prev$1 = types[0]; i$3 < len - 1; ++i$3) {
                ...
                if (type$2 == '+' && prev$1 == '1' && types[i$3 + 1] ==
                    '1') { types[i$3] = '1' } else if (type$2 == ',' &&
                    prev$1 == types[i$3 + 1] &&
            ...
            for (var i$5 = 0, cur$1 = outerType; i$5 < len; ++i$5) {
                ...
                if (cur$1 == 'L' && type$4 ==
                    '1') { types[i$5] = 'L' } else if (isStrong.test(
                    type$4)) { cur$1 = type$4 }
            }
            ...
            for (var i$6 = 0; i$6 < len; ++i$6) {
                if (isNeutral.test(types[i$6])) {
                    ...
                    var replace$1 = before == after
                        ? (before ? 'L' : 'R')
                        : outerType
                    for (var j$1 = i$6; j$1 <
                    end$1; ++j$1) { types[j$1] = replace$1 }
                    i$6 = end$1 - 1
                }
            }
...
// compatibility wrappers are needed.
    ...
    function e_button (e) {
        var b = e.which
        if (b == null) {
            if (e.button & 1) { b = 1 } else if (e.button &
                2) { b = 3 } else if (e.button & 4) { b = 2 }
        }
    ...
    function zeroWidthElement (measure) {
        if (zwspSupported == null) {
            ...
            if (measure.firstChild.offsetHeight != 0) {
                zwspSupported = test.offsetWidth <= 1 && test.offsetHeight >
                    2 && !(ie && ie_version < 8)
            }
        }
...
// load a mode. (Preferred mechanism is the require/define calls.)
    function defineMode (name, mode) {
        if (arguments.length >
            2) { mode.dependencies = Array.prototype.slice.call(arguments, 2) }
        modes[name] = mode
    }
...
// parsers more succinct.
    ...
    Context.prototype.baseToken = function (n) {
        ...
        while (this.baseTokens[this.baseTokenPos] <=
        n) { this$1.baseTokenPos += 2 }
...
// line.
    function highlightLine (cm, line, context, forceToEnd) {
        ...
        runMode(cm, line.text, cm.doc.mode, context,
            function (end, style) { return st.push(end, style) },
        ...
        var loop = function (o) {
            ...
            context.state = true
            runMode(cm, line.text, overlay.mode, context,
                function (end, style) {
                    var start = i
        ...
        return {
            styles: st,
            classes: lineClasses.bgClass || lineClasses.textClass
                ? lineClasses
                : null,
        }
    }
    ...
    function getContextBefore (cm, n, precise) {
        ...
        doc.iter(start, n, function (line) {
            ...
            display.viewFrom && pos < display.viewTo ? context.save() : null
            context.nextLine()
        })
...
// Utility for getTokenAt and getLineTokens
    function takeToken (cm, pos, precise, asArray) {
        ...
        while ((asArray || stream.pos < pos.ch) && !stream.eol()) {
            ...
            if (asArray) {
                tokens.push(new Token(stream, style,
    ...
    function extractLineClasses (type, output) {
        if (type) {
            for (; ;) {
                ...
                if (!lineClass) { break }
                type = type.slice(0, lineClass.index) +
                    type.slice(lineClass.index + lineClass[0].length)
                var prop = lineClass[1] ? 'bgClass' : 'textClass'
                if (output[prop] ==
                    null) { output[prop] = lineClass[2] } else if (!(new RegExp(
                    '(?:^|\s)' + lineClass[2] + '(?:$|\s)')).test(
        ...
        while (!stream.eol()) {
            if (stream.pos > cm.options.maxHighlightLength) {
                ...
                style = extractLineClasses(
                    readToken(mode, stream, context.state, inner), lineClasses)
            }
            if (inner) {
...
// parse correctly.
    function findStartLine (cm, n, precise) {
        ...
        for (var search = n; search > lim; --search) {
            ...
            if (after && (!precise || search +
                (after instanceof SavedContext ? after.lookAhead : 0) <=
                doc.modeFrontier)) { return search }
    ...
    function retreatFrontier (doc, n) {
        ...
        for (var line = n - 1; line > start; line--) {
            ...
            if (saved &&
                (!(saved instanceof SavedContext) || line + saved.lookAhead <
                    n)) {
                start = line + 1
                break
            }
...
// information about line-wide styles that were set by the mode.
    function buildLineContent (cm, lineView) {
        ...
        var content = eltP('span', null, null,
        ...
        for (var i = 0; i <= (lineView.rest ? lineView.rest.length : 0); i++) {
            ...
            if (builder.map.length == 0) {
                builder.map.push(0, 0, builder.content.appendChild(
        ...
        if (webkit) {
            ...
            if (/\bcm-tab\b/.test(last.className) || (last.querySelector &&
                last.querySelector(
                    '.cm-tab'))) { builder.content.className = 'cm-tab-wrap-hack' }
        }
...
// the line map. Takes care to render special characters separately.
    ...
    function splitSpaces (text, trailingBefore) {
        ...
        for (var i = 0; i < text.length; i++) {
            ...
            if (ch == ' ' && spaceBefore &&
                (i == text.length - 1 || text.charCodeAt(i + 1) ==
                    32)) { ch = '\u00a0' }
            result += ch
            spaceBefore = ch == ' '
        }
    ...
    function buildTokenBadBidi (inner, order) {
        return function (
            ...
            for (; ;) {
                ...
                if (part.to >= end) {
                    return inner(builder, text, style, startStyle, endStyle,
                        title, css)
                }
                inner(builder, text.slice(0, part.to - start), style,
                    startStyle, null, title, css)
                startStyle = null
        ...
        for (; ;) {
            if (nextChange == pos) { // Update current marker set
                ...
                for (var j = 0; j < spans.length; ++j) {
                    ...
                    if (m.type == 'bookmark' && sp.from == pos &&
                        ...
                        (sp.to == null || sp.to > pos || m.collapsed && sp.to ==
                            pos && sp.from == pos)) {
                        ...
                        if (m.startStyle && sp.from ==
                            pos) { spanStartStyle += ' ' + m.startStyle }
                        if (m.endStyle && sp.to == nextChange) {
                            (endStyles || (endStyles = [])).push(m.endStyle,
                                sp.to)
                        }
                ...
                if (endStyles) {
                    for (var j$1 = 0; j$1 < endStyles.length; j$1 += 2) {
                        if (endStyles[j$1 + 1] ==
                            nextChange) { spanEndStyle += ' ' + endStyles[j$1] }
                    }
                }
            ...
            while (true) {
                if (text) {
                    ...
                    if (!collapsed) {
                        var tokenText = end > upto
                            ? text.slice(0, upto - pos)
                            : text
                        builder.addToken(builder, tokenText,
...
// classes.
    ...
    function updateLineClasses (cm, lineView) {
        ...
        if (lineView.line.wrapClass) {
            ...
            lineView.text) { lineView.node.className = '' }
        ...
        if (lineView.line.gutterClass) {
            ...
            lineView.gutterBackground = elt('div', null,
                ...
                ('left: ' + (cm.options.fixedGutter
                        ? dims.fixedPos
                        : -dims.gutterTotalWidth) + 'px; width: ' +
                ...
                lineView.lineNumber = gutterWrap.appendChild(
                    elt('div', lineNumberFor(cm.options, lineN),
                        ...
                        ('left: ' +
                            ...
                            'px; width: ' + (cm.display.lineNumInnerWidth) +
                            'px')))
            }
            if (markers) {
                for (var k = 0; k < cm.options.gutters.length; ++k) {
                    ...
                    if (found) {
                        gutterWrap.appendChild(
                            elt('div', [found], 'CodeMirror-gutter-elt',
                                ('left: ' + (dims.gutterLeft[id]) +
                                    'px; width: ' + (dims.gutterWidth[id]) +
                                    'px')))
                    }
                }
            }
    ...
    function updateLineWidgets (cm, lineView, dims) {
        ...
        for (var node = lineView.node.firstChild, next = (void 0); node; node = next) {
            ...
            if (node.className ==
                'CodeMirror-linewidget') { lineView.node.removeChild(node) }
        }
...
// collapsed spans). The widgets for all of them need to be drawn.
    function insertLineWidgets (cm, lineView, dims) {
        ...
        if (lineView.rest) {
            for (var i = 0; i < lineView.rest.length; i++) {
                insertLineWidgetsFor(cm, lineView.rest[i], lineView, dims,
                    false)
            }
        }
    }
    ...
    function widgetHeight (widget) {
        ...
        if (!contains(document.body, widget.node)) {
            ...
            if (widget.noHScroll) {
                parentStyle += 'width: ' + cm.display.wrapper.clientWidth +
                    'px;'
            }
...
// POSITION MEASUREMENT
    ...
    function paddingH (display) {
        ...
        if (!isNaN(data.left) &&
            !isNaN(data.right)) { display.cachedPaddingH = data }
        return data
    }
...
// Find a line view that corresponds to the given line number.
    function findViewForLine (cm, lineN) {
        ...
        var ext = cm.display.externalMeasured
        if (ext && lineN >= ext.lineN && lineN < ext.lineN +
            ext.size) { return ext }
    }
...
// actual character (or fetches it from the cache).
    ...
    function nodeAndOffsetInLineMap (map, ch, bias) {
        ...
        for (var i = 0; i < map.length; i += 3) {
            ...
            if (start != null) {
                node = map[i + 2]
                if (mStart == mEnd && bias ==
                    (node.insertLeft ? 'left' : 'right')) { collapse = bias }
                if (bias == 'left' && start == 0) {
                    while (i && map[i - 2] == map[i - 3] &&
                    map[i - 1].insertLeft) {
                        node = map[(i -= 3) + 2]
                        collapse = 'left'
                    }
    ...
    function measureCharInner (cm, prepared, ch, bias) {
        ...
        var node = place.node, start = place.start, end = place.end,
        ...
        if (node.nodeType == 3) { // If it is a text node, use a range to retrieve the coordinates.
            for (var i$1 = 0; i$1 < 4; i$1++) { // Retry a maximum of 4 times when nonsense rectangles are returned
                while (start && isExtendingChar(prepared.line.text.charAt(
                    place.coverStart + start))) { --start }
                while (place.coverStart + end < place.coverEnd &&
                isExtendingChar(prepared.line.text.charAt(
                    place.coverStart + end))) { ++end }
                if (ie && ie_version < 9 && start == 0 && end ==
                    place.coverEnd -
                    place.coverStart) { rect = node.parentNode.getBoundingClientRect() } else {
                    rect = getUsefulRect(
                        range(node, start, end).getClientRects(), bias)
                }
            ...
            if (cm.options.lineWrapping &&
                ...
                rect = rects[bias == 'right'
                    ? rects.length - 1
                    : 0]
            } else { rect = node.getBoundingClientRect() }
        }
        if (ie && ie_version < 9 && !start &&
        ...
        var result = {
            left: (collapse == 'right' ? rect.right : rect.left) -
                prepared.rect.left,
...
// returned incorrectly when zoomed on IE10 and below.
    function maybeUpdateRectForZooming (measure, rect) {
        if (!window.screen || screen.logicalXDPI == null ||
            ...
            !hasBadZoomedRects(measure)) { return rect }
    ...
    function clearLineMeasurementCacheFor (lineView) {
        if (lineView.measure) {
            ...
            if (lineView.rest) {
                for (var i = 0; i <
                lineView.rest.length; i++) { lineView.measure.caches[i] = {} }
            }
        }
    }
...
// Context may be "window", "page", "div", or "local"./null.
    ...
    function charCoords (cm, pos, context, lineObj, bias) {
        ...
        return intoCoordSystem(cm, lineObj,
            measureChar(cm, lineObj, pos.ch, bias), context)
    }
...
// to the last character on the line.
    function cursorCoords (
        ...
        function get (ch, right) {
            var m = measureCharPrepared(cm, preparedMeasure, ch,
                right ? 'right' : 'left', varHeight)
            if (right) { m.left = m.right } else { m.right = m.left }
            return intoCoordSystem(cm, lineObj, m, context)
        }

        var order = getOrder(lineObj, cm.doc.direction), ch = pos.ch,
            sticky = pos.sticky
...
// Input must be lineSpace-local ("div" coordinate system).
    function coordsChar (cm, x, y) {
        ...
        if (lineN > last) {
            return PosWithInfo(doc.first + doc.size - 1,
                getLine(doc, last).text.length, null, true, 1)
        }
        if (x < 0) { x = 0 }
        ...
        for (; ;) {
            ...
            var collapsed = collapsedSpanAround(lineObj,
    ...
    function wrappedLineExtentChar (cm, lineObj, preparedMeasure, target) {
        ...
        var targetTop = intoCoordSystem(cm, lineObj,
            measureCharPrepared(cm, preparedMeasure, target), 'line').top
...
// coordinates, in top-to-bottom, left-to-right order.
    ...
    function coordsCharInner (cm, lineObj, lineNo, x, y) {
        ...
        if (boxAround) {
            ...
            sticky = ch == 0 ? 'after' : ch == lineObj.text.length ? 'before' :
                (measureCharPrepared(cm, preparedMeasure,
                    ch - (ltr ? 1 : 0)).bottom + widgetHeight <= y) == ltr ?
                    'after' : 'before'
            // Now get accurate coordinates for this place, in order to get a
            // base X position
            var coords = cursorCoords(cm, Pos(lineNo, ch, sticky), 'line',
                lineObj, preparedMeasure)
            baseX = coords.left
    ...
    function coordsBidiPart (
        ...
        var index = findFirst(function (i) {
            ...
            return boxIsAfter(cursorCoords(cm,
                Pos(lineNo, ltr ? part.to : part.from,
                    ltr ? 'before' : 'after'),
        ...
        if (index > 0) {
            ...
            if (boxIsAfter(start, x, y, true) && start.top >
                y) { part = order[index - 1] }
        }
        return part
    }
    ...
    function coordsBidiPartWrapped (
        ...
        for (var i = 0; i < order.length; i++) {
            ...
            var ltr = p.level != 1
            var endX = measureCharPrepared(cm, preparedMeasure,
                ltr ? Math.min(end, p.to) - 1 : Math.max(begin, p.from)).right
...
// Compute the default text height.
    function textHeight (display) {
        if (display.cachedTextHeight !=
            null) { return display.cachedTextHeight }
        if (measureText == null) {
            measureText = elt('pre')
...
// view, so that we don't interleave reading and writing to the DOM.
    function getDimensions (cm) {
        ...
        for (var n = d.gutters.firstChild, i = 0; n; n = n.nextSibling, ++i) {
            left[cm.options.gutters[i]] = n.offsetLeft + n.clientLeft +
                gutterLeft
...
// coordinates beyond the right of the text.
    function posFromMouse (cm, e, liberal, forRect) {
        ...
        if (!liberal && e_target(e).getAttribute('cm-not-content') ==
            'true') { return null }
        ...
        if (forRect && coords.xRel == 1 &&
            ...
            var colDiff = countColumn(line, line.length, cm.options.tabSize) -
                line.length
            coords = Pos(coords.line, Math.max(0, Math.round(
...
// Draws a cursor for the given range
    function drawSelectionCursor (cm, head, output) {
        ...
        if (pos.other) {
            ...
            otherCursor.style.height = (pos.other.bottom - pos.other.top) *
                .85 + 'px'
        }
    }
...
// Draws the given range as a highlighted selection
    function drawSelectionRange (cm, range, output) {
        ...
        var rightSide = Math.max(display.sizerWidth,
            displayWidth(cm) - display.sizer.offsetLeft) - padding.right
        ...
        function drawForLine (line, fromArg, toArg) {
            ...
            iterateBidiSections(order, fromArg || 0,
                toArg == null ? lineLen : toArg, function (from, to, dir, i) {
                    var ltr = dir == 'ltr'
                    ...
                    if (toPos.top - fromPos.top <= 3) { // Single line
                        ...
                        var left = openLeft ? leftSide : (ltr
                            ? fromPos
                            : toPos).left
                        var right = openRight ? rightSide : (ltr
                            ? toPos
                            : fromPos).right
                        ...
                        if (ltr) {
                            ...
                            topRight = !docLTR && openStart && first
                                ? rightSide
                                : fromPos.right
                            botLeft = !docLTR && openEnd && last
                                ? leftSide
                                : toPos.left
                            botRight = !docLTR ? rightSide : wrapX(to, dir,
                                'after')
                        }
                        add(topLeft, fromPos.top, topRight - topLeft,
                            fromPos.bottom)
                        if (fromPos.bottom < toPos.top) {
                            add(leftSide, fromPos.bottom, null, toPos.top)
                        }
                        add(botLeft, toPos.top, botRight - botLeft,
                            toPos.bottom)
                    }

                    if (!start || cmpCoords(fromPos, start) <
                        0) { start = fromPos }
        ...
        if (sFrom.line == sTo.line) {
            ...
            var leftEnd = drawForLine(sFrom.line, sFrom.ch,
                singleVLine ? fromLine.text.length + 1 : null).end
            ...
            if (singleVLine) {
                if (leftEnd.top < rightStart.top - 2) {
                    ...
                    add(leftEnd.right, leftEnd.top,
                        rightStart.left - leftEnd.right, leftEnd.bottom)
                }
            }
...
// Cursor-blinking
    ...
    function onFocus (cm, e) {
        ...
        if (!cm.state.focused) {
            ...
            if (!cm.curOp && cm.display.selForContextMenu != cm.doc.sel) {
                ...
                if (webkit) {
                    setTimeout(
                        function () { return cm.display.input.reset(true) },
                        20)
                } // Issue #1730
            }
    ...
    function onBlur (cm, e) {
        ...
        setTimeout(
            function () { if (!cm.state.focused) { cm.display.shift = false } },
            150)
    }
    ...
    function updateHeightsInViewport (cm) {
        ...
        for (var i = 0; i < display.view.length; i++) {
            ...
            if (diff > .005 || diff < -.005) {
                ...
                if (cur.rest) {
                    for (var j = 0; j <
                    cur.rest.length; j++) { updateWidgetHeight(cur.rest[j]) }
                }
            }
        }
    }
...
// height, and ensure (see op.scrollToPos) properties.
    function visibleLines (display, doc, viewport) {
        var top = viewport && viewport.top != null
...
// horizontal scrolling.
    function alignHorizontally (cm) {
        ...
        if (!display.alignWidgets &&
            (!display.gutters.firstChild || !cm.options.fixedGutter)) { return }
        ...
        for (var i = 0; i < view.length; i++) {
            if (!view[i].hidden) {
                ...
                if (align) {
                    for (var j = 0; j <
                    align.length; j++) { align[j].style.left = left }
                }
            }
        }
        if (cm.options.fixedGutter) {
...
// scrolled out of view, this ensures that the cursor is visible.
    function maybeScrollWindow (cm, rect) {
        ...
        var display = cm.display, box = display.sizer.getBoundingClientRect(),
            doScroll = null
        if (rect.top + box.top < 0) { doScroll = true } else if (rect.bottom +
        ...
        if (doScroll != null && !phantom) {
            var scrollNode = elt('div', '\u200b', null,
                ('position: absolute;\n                         top: ' +
                    ...
                    'px;\n                         height: ' +
                    (rect.bottom - rect.top + scrollGap(cm) +
                        display.barHeight) +
                    'px;\n                         left: ' + (rect.left) +
                    'px; width: ' + (Math.max(2, rect.right - rect.left)) +
                    'px;'))
...
// measured, the position of something may 'drift' during drawing).
    function scrollPosIntoView (cm, pos, end, margin) {
        ...
        if (!cm.options.lineWrapping && pos == end) {
            ...
            pos = pos.ch ? Pos(pos.line,
                pos.sticky == 'before' ? pos.ch - 1 : pos.ch, 'after') : pos
            end = pos.sticky == 'before'
        ...
        for (var limit = 0; limit < 5; limit++) {
            ...
            if (scrollPos.scrollTop != null) {
                ...
                if (Math.abs(cm.doc.scrollTop - startTop) >
                    1) { changed = true }
            }
...
// vertical/horizontal position does not need to be adjusted.
    function calculateScrollPos (cm, rect) {
        ...
        var screenw = displayWidth(cm) -
            (cm.options.fixedGutter ? display.gutters.offsetWidth : 0)
        var tooWide = rect.right - rect.left > screenw
        if (tooWide) { rect.right = rect.left + screenw }
        if (rect.left < 10) { result.scrollLeft = 0 } else if (rect.left <
            screenleft) {
            result.scrollLeft = Math.max(0, rect.left - (tooWide ? 0 : 10))
        } else if (rect.right > screenw + screenleft -
            3) { result.scrollLeft = rect.right + (tooWide ? 0 : 10) - screenw }
        return result
    }
...
// operation (to be applied when the operation finishes).
    function addToScrollTop (cm, top) {
        ...
        cm.curOp.scrollTop = (cm.curOp.scrollTop == null
            ? cm.doc.scrollTop
...
// covers the visible area.
    ...
    function setScrollTop (cm, val, forceScroll) {
        ...
        if (cm.display.scroller.scrollTop !=
            val) { cm.display.scroller.scrollTop = val }
    }
...
// aligned.
    function setScrollLeft (cm, val, isScroller, forceScroll) {
        ...
        if ((isScroller ? val == cm.doc.scrollLeft : Math.abs(
            cm.doc.scrollLeft - val) < 2) && !forceScroll) { return }
        cm.doc.scrollLeft = val
        alignHorizontally(cm)
        if (cm.display.scroller.scrollLeft !=
            val) { cm.display.scroller.scrollLeft = val }
...
// shot to minimize update/measure roundtrips.
    ...
    var NativeScrollbars = function (place, scroll, cm) {
        ...
        var vert = this.vert = elt('div',
            [elt('div', null, null, 'min-width: 1px')], 'CodeMirror-vscrollbar')
        ...
        if (ie && ie_version <
            8) { this.horiz.style.minHeight = this.vert.style.minWidth = '18px' }
    }
    ...
    NativeScrollbars.prototype.enableZeroWidthBar = function (
        ...
        function maybeDisable () {
            ...
            var elt = type == 'vert' ? document.elementFromPoint(box.right - 1,
    ...
    function updateScrollbars (cm, measure) {
        ...
        startHeight != cm.display.barHeight; i++) {
            if (startWidth != cm.display.barWidth &&
                cm.options.lineWrapping) { updateHeightsInViewport(cm) }
...
// content.
    function updateScrollbarsInner (cm, measure) {
        ...
        d.heightForcer.style.borderBottom = sizes.bottom +
    ...
    function initScrollbars (cm) {
        ...
        cm.display.scrollbars = new scrollbarModel[cm.options.scrollbarStyle](
            function (node) {
                cm.display.wrapper.insertBefore(node,
                    cm.display.scrollbarFiller)
                // Prevent clicks in the scrollbars from killing focus
                on(node, 'mousedown', function () {
                    if (cm.state.focused) {
                        setTimeout(
                            function () { return cm.display.input.focus() }, 0)
                    }
                })
...
// Finish an operation, updating the display and signalling delayed events
    function endOperation (cm) {
        ...
        finishOperation(op, function (group) {
            for (var i = 0; i <
            group.ops.length; i++) { group.ops[i].cm.curOp = null }
            endOperations(group)
        })
    }
...
// that the minimum number of relayouts are required.
        ...
        if (display.maxLineChanged && !cm.options.lineWrapping) {
            ...
            op.barMeasure.scrollWidth =
                Math.max(display.scroller.clientWidth,
                    display.sizer.offsetLeft + op.adjustWidthTo +
                    scrollGap(cm) + cm.display.barWidth)
            op.maxScrollLeft = Math.max(0,
                display.sizer.offsetLeft + op.adjustWidthTo - displayWidth(cm))
        }

        if (op.updatedDisplay ||
            op.selectionChanged) { op.preparedSelection = display.input.prepareSelection() }
    }
    ...
    function endOperation_W2 (op) {
        ...
        if (op.updatedDisplay || op.startHeight !=
            cm.doc.height) { updateScrollbars(cm, op.barMeasure) }
    ...
    function endOperation_finish (op) {
        ...
        if (display.wheelStartX != null &&
            (op.scrollTop != null || op.scrollLeft != null ||
                op.scrollToPos)) { display.wheelStartX = display.wheelStartY = null }
    ...
    function regChange (cm, from, to, lendiff) {
        ...
        if (lendiff && to < display.viewTo &&
            (display.updateLineNumbers == null || display.updateLineNumbers >
                from)) { display.updateLineNumbers = from }

        cm.curOp.viewChanged = true

        if (from >= display.viewTo) { // Change after
            if (sawCollapsedSpans && visualLineNo(cm.doc, from) <
                display.viewTo) { resetView(cm) }
        ...
        if (ext) {
            if (to < ext.lineN) { ext.lineN += lendiff } else if (from <
                ext.lineN + ext.size) { display.externalMeasured = null }
        }
    }
...
// "gutter", "class", "widget"
    function regLineChange (cm, line, type) {
        ...
        if (ext && line >= ext.lineN && line < ext.lineN +
            ext.size) { display.externalMeasured = null }
...
// Clear the view.
    ...
    function viewCuttingPoint (cm, oldN, newN, dir) {
        ...
        if (!sawCollapsedSpans || newN == cm.doc.first +
            cm.doc.size) { return { index: index, lineN: newN } }
    ...
    function adjustView (cm, from, to) {
        ...
        if (view.length == 0 || from >= display.viewTo || to <=
            ...
            if (display.viewFrom > from) {
                display.view = buildViewArray(cm, from, display.viewFrom).
                    concat(display.view)
        ...
        doc.iter(context.line,
            ...
            function (line) {
                if (context.line >= cm.display.viewFrom) { // Visible
                    ...
                    var ischange = !oldStyles || oldStyles.length !=
                        line.styles.length ||
                        oldCls != newCls &&
                        (!oldCls || !newCls || oldCls.bgClass !=
                            newCls.bgClass || oldCls.textClass !=
                            newCls.textClass)
                    ...
                    line.stateAfter = context.line % 5 == 0
                        ? context.save()
                        : null
                    context.nextLine()
                }
        ...
        for (var first = true; ; first = false) {
            if (!first || !cm.options.lineWrapping || update.oldDisplayWidth ==
                ...
                if (viewport && viewport.top != null) {
                    viewport = {
                        top: Math.min(cm.doc.height + paddingVert(cm.display) -
                ...
                if (update.visible.from >= cm.display.viewFrom &&
                    update.visible.to <= cm.display.viewTo) { break }
            }
    ...
    function patchDisplay (cm, updateNumbersFrom, dims) {
        ...
        function rm (node) {
            ...
            if (webkit && mac && cm.display.currentWheelTarget ==
                node) { node.style.display = 'none' } else {
        ...
        for (var i = 0; i < view.length; i++) {
            var lineView = view[i]
            if (lineView.hidden) {
            } else if (!lineView.node || lineView.node.parentNode !=
                container) { // Not drawn yet
                ...
                if (lineView.changes) {
                    if (indexOf(lineView.changes, 'gutter') >
                        -1) { updateNumber = false }
                    updateLineForChanges(cm, lineView, lineN, dims)
                }
                if (updateNumber) {
                    removeChildren(lineView.lineNumber)
                    lineView.lineNumber.appendChild(document.createTextNode(
...
// scroll (if it is large enough).
    if (ie) { wheelPixelsPerUnit = -.53 } else if (gecko) { wheelPixelsPerUnit = 15 } else if (chrome) { wheelPixelsPerUnit = -.7 } else if (safari) {
    ...
    function wheelEventDelta (e) {
        ...
        if (dy == null && e.detail && e.axis ==
            e.VERTICAL_AXIS) { dy = e.detail } else if (dy ==
            null) { dy = e.wheelDelta }
        return { x: dx, y: dy }
    }
    ...
    function onScrollWheel (cm, e) {
        ...
        if (dy && mac && webkit) {
            outer: for (var cur = e.target, view = display.view; cur !=
            scroll; cur = cur.parentNode) {
                for (var i = 0; i < view.length; i++) {
                    if (view[i].node == cur) {
        ...
        if (wheelSamples < 20) {
            if (display.wheelStartX == null) {
                ...
                setTimeout(function () {
                    ...
                    if (!sample) { return }
                    wheelPixelsPerUnit = (wheelPixelsPerUnit * wheelSamples +
...
// view, that getCursor returns, etc).
    ...
    Selection.prototype.somethingSelected = function () {
        ...
        for (var i = 0; i <
        this.ranges.length; i++) { if (!this$1.ranges[i].empty()) { return true } }
        return false
    }
...
// same text, or the end of the change if the change covers it.
    function adjustForChange (pos, change) {
        ...
        var line = pos.line + change.text.length -
            (change.to.line - change.from.line) - 1, ch = pos.ch
...
// Perform a change on the document data structure.
    function updateDoc (doc, change, markedSpans, estimateHeight) {
        ...
        var lastText = lst(text), lastSpans = spansFor(text.length - 1),
        ...
        if (change.full) {
            ...
            if (text.length == 1) {
                ...
                added$1.push(
                    new Line(lastText + firstLine.text.slice(to.ch), lastSpans,
                        estimateHeight))
...
// object.
    function historyChangeFromChange (doc, change) {
        ...
        linkedDocs(doc, function (doc) {
            return attachLocalSpans(doc, histChange, change.from.line,
                change.to.line + 1)
        }, true)
        return histChange
    }
...
// allows merging (starting with "+") into a single event.
    function addChangeToHistory (doc, change, selAfter, opId) {
        ...
        if ((hist.lastOp == opId ||
            ...
            if (cmp(change.from, change.to) == 0 && cmp(change.from, last.to) ==
                0) {
        ...
        if (opId == hist.lastSelOp ||
            (origin && hist.lastSelOrigin == origin &&
                (hist.lastModTime == hist.lastSelTime && hist.lastOrigin ==
                    origin ||
                    selectionEventCanBeMerged(doc, origin, lst(hist.done),
                        sel)))) {
...
// Used to store marked span information in the history.
    function attachLocalSpans (doc, change, from, to) {
        ...
        doc.iter(Math.max(doc.first, from), Math.min(doc.first + doc.size, to),
            function (line) {
                if (line.markedSpans) {
                    (existing || (existing = change['spans_' +
    ...
    function copyHistoryArray (events, newGroup, instantiateSel) {
        ...
        for (var i = 0; i < events.length; ++i) {
            ...
            for (var j = 0; j < changes.length; ++j) {
                ...
                newChanges.push(
                    { from: change.from, to: change.to, text: change.text })
                if (newGroup) {
...
// Extend the primary selection range, discard the rest.
    function extendSelection (doc, head, other, options, extend) {
        ...
        setSelection(doc,
            new Selection([extendRange(doc.sel.primary(), head, other, extend)],
                0), options)
    }
...
// Set a new selection.
    ...
    function setSelectionNoUndo (doc, sel, options) {
        ...
        if (!(options && options.scroll === false) &&
            doc.cm) { ensureCursorVisible(doc.cm) }
    }
    ...
    function skipAtomicInSelection (doc, sel, bias, mayClear) {
        ...
        for (var i = 0; i < sel.ranges.length; i++) {
            ...
            var old = sel.ranges.length == doc.sel.ranges.length &&
                doc.sel.ranges[i]
            var newAnchor = skipAtomic(doc, range.anchor, old && old.anchor,
                bias, mayClear)
            var newHead = skipAtomic(doc, range.head, old && old.head, bias,
                mayClear)
        ...
        if (line.markedSpans) {
            for (var i = 0; i < line.markedSpans.length; ++i) {
                ...
                if ((sp.from == null ||
                    ...
                    (sp.to == null ||
                        (m.inclusiveRight ? sp.to >= pos.ch : sp.to >
                            pos.ch))) {
                    if (mayClear) {
                    ...
                    if (oldPos) {
                        ...
                        if (dir < 0
                            ? m.inclusiveRight
                            : m.inclusiveLeft) {
                            near = movePos(doc, near, -dir,
                                near && near.line == pos.line ? line : null)
                        }
                        if (near && near.line == pos.line &&
                            (diff = cmp(near, oldPos)) &&
                            (dir < 0 ? diff < 0 : diff > 0)) {
                            return skipAtomicInner(doc, near, pos, dir,
                                mayClear)
                        }
                    }
...
// Ensure a given position is not inside an atomic range.
    ...
    function movePos (doc, pos, dir, line) {
        ...
        } else if (dir > 0 && pos.ch ==
            (line || getLine(doc, pos.line)).text.length) {
...
// history, and propagating it to all linked documents.
    function makeChange (doc, change, ignoreReadOnly) {
        if (doc.cm) {
            if (!doc.cm.curOp) {
                return operation(doc.cm, makeChange)(doc, change,
                    ignoreReadOnly)
            }
    ...
    function makeChangeInner (doc, change) {
        ...
        addChangeToHistory(doc, change, selAfter,
...
// above or below them in the parent document.
    function shiftDoc (doc, distance) {
        ...
        if (doc.cm) {
            ...
            for (var d = doc.cm.display, l = d.viewFrom; l <
            d.viewTo; l++) { regLineChange(doc.cm, l, 'gutter') }
        }
    }
...
// that this document is part of.
    function makeChangeSingleDocInEditor (cm, change, spans) {
        var doc = cm.doc, display = cm.display, from = change.from,
            to = change.to
        ...
        if (doc.sel.contains(change.from, change.to) >
            -1) { signalCursorActivity(cm) }

        updateDoc(doc, change, spans, estimateHeight(cm))

        if (!cm.options.lineWrapping) {
            doc.iter(checkWidthStart, from.line + change.text.length,
                function (line) {
                    var len = lineLength(line)
        ...
        if (change.full) { regChange(cm) } else if (from.line == to.line &&
            change.text.length == 1 && !isWholeLineUpdate(cm.doc, change)) {
...
// shared position objects being unsafely updated.
    ...
    function rebaseHist (hist, change) {
        var from = change.from.line, to = change.to.line,
...
// See also http://marijnhaverbeke.nl/blog/codemirror-line-tree.html
    ...
    BranchChunk.prototype = {
        ...
        collapse: function collapse (lines) {
            ...
            for (var i = 0; i <
            this.children.length; ++i) { this$1.children[i].collapse(lines) }
        },
...
// Line widgets are block elements displayed above or below a line.
    ...
    LineWidget.prototype.clear = function () {
        ...
        var cm = this.doc.cm, ws = this.line.widgets, line = this.line,
            no = lineNo(line)
    ...
    function adjustScrollWhenAboveVisible (cm, line, diff) {
        if (heightAtLine(line) < ((cm.curOp && cm.curOp.scrollTop) ||
            cm.doc.scrollTop)) { addToScrollTop(cm, diff) }
    }
...
// Clear the marker.
    TextMarker.prototype.clear = function () {
        ...
        for (var i = 0; i < this.lines.length; ++i) {
            ...
            if (span.from == null && this$1.collapsed &&
                !lineIsHidden(this$1.doc, line) && cm) {
        ...
        if (!this.lines.length && this.doc.cm) {
            ...
            if (!op.maybeHiddenMarkers ||
                ...
                (op.maybeUnhiddenMarkers ||
                    (op.maybeUnhiddenMarkers = [])).push(this)
            }
        }
    ...
    function markText (doc, from, to, options, type) {
        ...
        doc.iter(curLine, to.line + 1, function (line) {
            if (cm && marker.collapsed && !cm.options.lineWrapping &&
                visualLine(line) == cm.display.maxLine) { updateMaxLine = true }
        ...
        if (marker.readOnly) {
            ...
            if (doc.history.done.length ||
                doc.history.undone.length) { doc.clearHistory() }
        }
        if (marker.collapsed) {
...
// markers.
    ...
    SharedTextMarker.prototype.clear = function () {
        ...
        this.explicitlyCleared = true
        for (var i = 0; i <
        this.markers.length; ++i) { this$1.markers[i].clear() }
    ...
    function markTextShared (doc, from, to, options, type) {
        ...
        linkedDocs(doc, function (doc) {
            ...
            markers.push(
                markText(doc, clipPos(doc, from), clipPos(doc, to), options,
                    type))
            for (var i = 0; i <
            doc.linked.length; ++i) { if (doc.linked[i].isParent) { return } }
            primary = lst(markers)
        })
    ...
    function detachSharedMarkers (markers) {
        var loop = function (i) {
            ...
            linkedDocs(marker.primary.doc,
                function (d) { return linked.push(d) })
    ...
    Doc.prototype = createObj(BranchChunk.prototype, {
        ...
        getCursor: function (start) {
            ...
            if (start == null || start ==
                'head') { pos = range.head } else if (start ==
                'anchor') { pos = range.anchor } else if (start == 'end' ||
                start == 'to' || start ===
                false) { pos = range.to() } else { pos = range.from() }
            return pos
        },
        ...
        setCursor: docMethodOp(function (line, ch, options) {
            setSimpleSelection(this, clipPos(this,
                    typeof line == 'number' ? Pos(line, ch || 0) : line), null,
                options)
        }),
        setSelection: docMethodOp(function (anchor, head, options) {
            setSimpleSelection(this, clipPos(this, anchor),
                clipPos(this, head || anchor), options)
        }),
        extendSelection: docMethodOp(function (head, other, options) {
            extendSelection(this, clipPos(this, head),
                other && clipPos(this, other), options)
        }),
        ...
        addSelection: docMethodOp(function (anchor, head, options) {
            ...
            setSelection(this, normalizeSelection(ranges, ranges.length - 1),
                options)
        }),
        ...
        undoSelection: docMethodOp(
            function () {makeChangeFromHistory(this, 'undo', true)}),
        redoSelection: docMethodOp(
            function () {makeChangeFromHistory(this, 'redo', true)}),
        ...
        historySize: function () {
            ...
            for (var i = 0; i <
            hist.done.length; i++) { if (!hist.done[i].ranges) { ++done } }
            for (var i$1 = 0; i$1 <
            hist.undone.length; i$1++) { if (!hist.undone[i$1].ranges) { ++undone } }
            ...
            this.iter(function (line) {
                if (line.gutterMarkers && line.gutterMarkers[gutterID]) {
                    changeLine(this$1, line, 'gutter', function () {
                        ...
                        if (isEmpty(
                            line.gutterMarkers)) { line.gutterMarkers = null }
                        return true
                    })
                }
            })
        ...
        addLineClass: docMethodOp(function (handle, where, cls) {
            return changeLine(this, handle,
                where == 'gutter' ? 'gutter' : 'class', function (line) {
                    var prop = where == 'text' ? 'textClass'
                        : where == 'background' ? 'bgClass'
                            : where == 'gutter' ? 'gutterClass' : 'wrapClass'
                    if (!line[prop]) { line[prop] = cls } else if (classTest(
                        cls).
                        test(line[prop])) { return false } else {
                        line[prop] += ' ' + cls
                    }
                    return true
                })
                ...
                where == 'gutter' ? 'gutter' : 'class', function (line) {
                    var prop = where == 'text' ? 'textClass'
                        : where == 'background' ? 'bgClass'
                            : where == 'gutter' ? 'gutterClass' : 'wrapClass'
                    var cur = line[prop]
                    if (!cur) { return false } else if (cls ==
                        null) { line[prop] = null } else {
                        var found = cur.match(classTest(cls))
                        if (!found) { return false }
                        var end = found.index + found[0].length
                        line[prop] = cur.slice(0, found.index) +
        ...
        linkedDoc: function (options) {
            ...
            var copy = new Doc(getLines(this, from, to),
                options.mode || this.modeOption, from, this.lineSep,
                this.direction)
            ...
            (this.linked || (this.linked = [])).push(
                { doc: copy, sharedHist: options.sharedHist })
            copy.linked = [
                {
                    doc: this,
                    isParent: true,
        ...
        unlinkDoc: function (other) {
            ...
            if (other.history == this.history) {
                ...
                linkedDocs(other,
                    function (doc) { return splitIds.push(doc.id) }, true)
                other.history = new History(null)
                other.history.done = copyHistoryArray(this.history.done,
                    splitIds)
        ...
        if (files && files.length && window.FileReader && window.File) {
            ...
            var loadFile = function (file, i) {
                if (cm.options.allowDropFileTypes &&
                    indexOf(cm.options.allowDropFileTypes, file.type) ==
                    -1) { return }

                var reader = new FileReader
                reader.onload = operation(cm, function () {
                    var content = reader.result
                    if (/[\x00-\x08\x0e-\x1f]{2}/.test(
                        content)) { content = '' }
                    text[i] = content
                    if (++read == n) {
                        pos = clipPos(cm.doc, pos)
            ...
            try {
                ...
                if (text$1) {
                    ...
                    if (cm.state.draggingText &&
                        !cm.state.draggingText.copy) { selected = cm.listSelections() }
    ...
    function normalizeKeyName (name) {
        ...
        for (var i = 0; i < parts.length - 1; i++) {
            ...
            if (/^(cmd|meta|m)$/i.test(
                mod)) { cmd = true } else if (/^a(lt)?$/i.test(
                mod)) { alt = true } else if (/^(c|ctrl|control)$/i.test(
                mod)) { ctrl = true } else if (/^s(hift)?$/i.test(
                mod)) { shift = true } else {
...
// this.
    function normalizeKeyMap (keymap) {
        ...
        for (var keyname in keymap) {
            if (keymap.hasOwnProperty(keyname)) {
                ...
                if (/^(name|fallthrough|(de|at)tach)$/.test(
                    keyname)) { continue }
                if (value == '...') {
                ...
                for (var i = 0; i < keys.length; i++) {
                    ...
                    var prev = copy[name]
                    if (!prev) { copy[name] = val } else if (prev != val) {
    ...
    function lookupKey (key, map, handle, context) {
        ...
        if (map.fallthrough) {
            if (Object.prototype.toString.call(map.fallthrough) !=
                '[object Array]') {
...
// backspace, delete, and similar functionality.
    ...
    function moveLogically (line, start, dir) {
        ...
        return ch == null ? null : new Pos(start.line, ch,
            dir < 0 ? 'after' : 'before')
    }
    ...
    function endOfLine (visually, cm, lineObj, lineNo, dir) {
            ...
            if (order) {
                ...
                if (part.level > 0 || cm.doc.direction == 'rtl') {
                    ...
                    ch = findFirst(function (ch) {
                            return measureCharPrepared(cm, prep, ch).top ==
                                targetTop
        ...
        return new Pos(lineNo, dir < 0 ? lineObj.text.length : 0,
            dir < 0 ? 'before' : 'after')
    }
    ...
    function moveVisually (cm, line, start, dir) {
        ...
        var mv = function (pos, dir) {
            return moveCharLogically(line, pos instanceof Pos ? pos.ch : pos,
                dir)
        }
        var prep
        ...
        if (cm.doc.direction == 'rtl' || part.level == 1) {
            ...
            if (ch != null && (!moveInStorageOrder ? ch >= part.from && ch >=
                wrappedLineExtent.begin : ch <= part.to && ch <=
        ...
        var searchInVisualLine = function (partPos, dir, wrappedLineExtent) {
            ...
            for (; partPos >= 0 && partPos < bidi.length; partPos += dir) {
                ...
                var moveInStorageOrder = (dir > 0) == (part.level != 1)
                var ch = moveInStorageOrder ? wrappedLineExtent.begin : mv(
                    wrappedLineExtent.end, -1)
        ...
        var nextCh = dir > 0 ? wrappedLineExtent.end : mv(
            wrappedLineExtent.begin, -1)
...
// editor, mostly used for keybindings.
    var commands = {
        ...
        singleSelection: function (cm) {
            return cm.setSelection(cm.getCursor('anchor'), cm.getCursor('head'),
                sel_dontScroll)
        },
        ...
        goLineStart: function (cm) {
            return cm.extendSelectionsBy(
                function (range) { return lineStart(cm, range.head.line) },
                { origin: '+move', bias: 1 },
            )
        },
        goLineStartSmart: function (cm) {
            return cm.extendSelectionsBy(
                function (range) { return lineStartSmart(cm, range.head) },
                { origin: '+move', bias: 1 },
            )
        },
        goLineEnd: function (cm) {
            return cm.extendSelectionsBy(
                function (range) { return lineEnd(cm, range.head.line) },
                { origin: '+move', bias: -1 },
            )
        },
        ...
        insertSoftTab: function (cm) {
            var spaces = [], ranges = cm.listSelections(),
            ...
            return runInOp(cm, function () {
                ...
                for (var i = 0; i < ranges.length; i++) {
                    ...
                    if (line) {
                        ...
                        if (cur.ch > 0) {
                            cur = new Pos(cur.line, cur.ch + 1)
                            cm.replaceRange(line.charAt(cur.ch - 1) +
                            ...
                            if (prev) {
                                ...
                                cm.replaceRange(
                                    line.charAt(0) + cm.doc.lineSeparator() +
...
// for bound mouse clicks.
    ...
    function dispatchKey (cm, name, e, handle) {
        ...
        if (seq) {
            ...
            if (dispatchKeyInner(cm, seq + ' ' + name, e,
                handle)) { return true }
        }
...
// Handle a key from the keydown event.
    function handleKeyBinding (cm, e) {
        ...
        if (e.shiftKey && !cm.state.keySeq) {
            ...
            return dispatchKey(cm, 'Shift-' + name, e,
...
// Handle a key from the keypress event
    ...
    function onKeyDown (e) {
        ...
        if (code == 18 && !/\bCodeMirror-crosshair\b/.test(
    ...
    function onKeyPress (e) {
        var cm = this
        if (eventInWidget(cm.display, e) || signalDOMEvent(cm, e) ||
            e.ctrlKey && !e.altKey || mac && e.metaKey) { return }
...
// not interfere with, such as a scrollbar or widget.
    function onMouseDown (e) {
        ...
        var pos = posFromMouse(cm, e), button = e_button(e),
        ...
        if (button == 1) {
            if (pos) { leftButtonDown(cm, pos, repeat, e) } else if (e_target(
                e) == display.scroller) { e_preventDefault(e) }
        } else if (button == 2) {
    ...
    function handleMappedButton (cm, button, pos, repeat, event) {
        var name = 'Click'
        if (repeat == 'double') { name = 'Double' + name } else if (repeat ==
            'triple') { name = 'Triple' + name }
        name = (button == 1 ? 'Left' : button == 2 ? 'Middle' : 'Right') + name

        return dispatchKey(cm, addModifierNames(name, event), event,
            function (bound) {
    ...
    function configureMouse (cm, repeat, event) {
        ...
        if (value.unit == null) {
            ...
            value.unit = rect ? 'rectangle' : repeat == 'single'
                ? 'char'
                : repeat == 'double' ? 'word' : 'line'
        }
        if (value.extend == null ||
            cm.doc.extend) { value.extend = cm.doc.extend || event.shiftKey }
...
// happen, and treat as a click if it didn't.
    ...
    function rangeForUnit (cm, pos, unit) {
        ...
        if (unit == 'line') {
            return new Range(Pos(pos.line, 0),
...
// Normal selection, as opposed to text dragging.
    function leftButtonSelect (cm, event, start, behavior) {
        ...
        if (behavior.addNew && !behavior.extend) {
            ...
            if (ourIndex > -1) { ourRange = ranges[ourIndex] } else {
        ...
        if (!behavior.addNew) {
            ...
            setSelection(doc, normalizeSelection(
        ...
        function extendTo (pos) {
            ...
            if (behavior.unit == 'rectangle') {
                ...
                var startCol = countColumn(getLine(doc, start.line).text,
                    start.ch, tabSize)
                var posCol = countColumn(getLine(doc, pos.line).text, pos.ch,
                    tabSize)
                var left = Math.min(startCol, posCol),
                    right = Math.max(startCol, posCol)
                for (var line = Math.min(start.line, pos.line), end = Math.min(
                    cm.lastLine(), Math.max(start.line, pos.line));
                     line <= end; line++) {
                    ...
                    if (left == right) {
                        ...
                        ranges.push(new Range(Pos(line, leftPos),
                ...
                ranges$1[ourIndex] = bidiSimplify(cm,
                    new Range(clipPos(doc, anchor), head))
                setSelection(doc, normalizeSelection(ranges$1, ourIndex),
                    sel_mouse)
            }
        }
            ...
            if (cmp(cur, lastPos) != 0) {
                ...
                if (cur.line >= visible.to || cur.line < visible.from) {
                    setTimeout(operation(cm,
                            function () {if (counter == curCount) { extend(e) }}),
                        150)
                }
            } else {
...
// of a bidi jump depending on the visual position of the head.
    function bidiSimplify (cm, range) {
        ...
        if (cmp(anchor, head) == 0 && anchor.sticky ==
        ...
        var boundary = index +
            ((part.from == anchor.ch) == (part.level != 1) ? 0 : 1)
        ...
        if (head.line != anchor.line) {
            leftSide = (head.line - anchor.line) *
                (cm.doc.direction == 'ltr' ? 1 : -1) > 0
        } else {
            var headIndex = getBidiPartAt(order, head.ch, head.sticky)
            var dir = headIndex - index || (head.ch - anchor.ch) *
                (part.level == 1 ? -1 : 1)
            if (headIndex == boundary - 1 || headIndex ==
                boundary) { leftSide = dir < 0 } else { leftSide = dir > 0 }
        }
        ...
        return anchor.ch == ch && anchor.sticky == sticky ? range : new Range(
            new Pos(anchor.line, ch, sticky), head)
    }
...
// handlers for the corresponding event.
    function gutterEvent (cm, e, type, prevent) {
        ...
        if (mX >= Math.floor(
            cm.display.gutters.getBoundingClientRect().right)) { return false }
        ...
        if (mY > lineBox.bottom ||
            !hasHandler(cm, type)) { return e_defaultPrevented(e) }
...
// right-click take effect on it.
    function onContextMenu (cm, e) {
        if (eventInWidget(cm.display, e) ||
            contextMenuInGutter(cm, e)) { return }
    ...
    function defineOptions (CodeMirror) {
        ...
        option('value', '', function (cm, val) { return cm.setValue(val) },
            true)
        ...
        option('lineSeparator', null, function (cm, val) {
            ...
            for (var i = newBreaks.length - 1; i >= 0; i--) {
                replaceRange(cm.doc, val, newBreaks[i],
        ...
        option('inputStyle', mobile ? 'contenteditable' : 'textarea',
            function () {
                throw new Error(
                    'inputStyle can not (yet) be changed in a running editor') // FIXME
            }, true)
        option('spellcheck', false,
            function (cm, val) { return cm.getInputField().spellcheck = val },
            true)
        ...
        option('fixedGutter', true, function (cm, val) {
            cm.display.gutters.style.left = val ? compensateForHScroll(
                cm.display) + 'px' : '0'
            cm.refresh()
        }, true)
        option('coverGutterNextToScrollbar', false,
            function (cm) { return updateScrollbars(cm) }, true)
        ...
        option('disableInput', false,
            function (cm, val) {if (!val) { cm.display.input.reset() }}, true)
        ...
        option('undoDepth', 200,
            function (cm, val) { return cm.doc.history.undoDepth = val })
        option('historyEventDelay', 1250)
        option('viewportMargin', 10, function (cm) { return cm.refresh() },
            true)
        ...
        option('direction', 'ltr',
            function (cm, val) { return cm.doc.setDirection(val) }, true)
        option('phrases', null)
    }
...
// that user code is usually dealing with.
    ...
    function CodeMirror (place, options) {
        ...
        if (ie && ie_version < 11) {
            setTimeout(function () { return this$1.display.input.reset(true) },
                20)
        }
        ...
        if (webkit && options.lineWrapping &&
            ...
            'optimizelegibility') { display.lineDiv.style.textRendering = 'auto' }
    }
...
// Attach the necessary event handlers when initializing the editor
    function registerEventHandlers (cm) {
        ...
        if (!captureRightClick) {
            on(d.scroller, 'contextmenu',
                function (e) { return onContextMenu(cm, e) })
        }
        ...
        function finishTouch () {
            if (d.activeTouch) {
                touchFinished = setTimeout(
                    function () { return d.activeTouch = null }, 1000)
        ...
        on(d.scroller, 'touchend', function (e) {
            ...
            if (touch && !eventInWidget(d, e) && touch.left != null &&
                ...
                { range = new Range(pos, pos) } else if (!touch.prev.prev ||
                    farAway(touch, touch.prev.prev)) // Double tap
                { range = cm.findWordAt(pos) } else // Triple tap
                {
                    range = new Range(Pos(pos.line, 0),
                        clipPos(cm.doc, Pos(pos.line + 1, 0)))
                }
        ...
        on(d.scroller, 'mousewheel',
            function (e) { return onScrollWheel(cm, e) })
        on(d.scroller, 'DOMMouseScroll',
            function (e) { return onScrollWheel(cm, e) })

        // Prevent wrapper from ever scrolling
        on(d.wrapper, 'scroll',
            function () { return d.wrapper.scrollTop = d.wrapper.scrollLeft = 0 })
...
// are left alone.
    function indentLine (cm, n, how, aggressive) {
        ...
        if (how == 'prev') {
            if (n > doc.first) {
                indentation = countColumn(getLine(doc, n - 1).text, null,
                    tabSize)
            } else { indentation = 0 }
        } else if (how == 'add') {
        ...
        if (indentString != curSpaceString) {
            replaceRange(doc, indentString, Pos(n, 0),
                Pos(n, curSpaceString.length), '+input')
...
// text was made out of.
    ...
    function applyTextInput (cm, inserted, deleted, sel, origin) {
        ...
        for (var i$1 = sel.ranges.length - 1; i$1 >= 0; i$1--) {
            ...
            if (range.empty()) {
                ...
                {
                    to = Pos(to.line,
                        Math.min(getLine(doc, to.line).text.length,
                            to.ch + lst(textLines).length))
                } else if (lastCopied && lastCopied.lineWise &&
    ...
    function triggerElectric (cm, inserted) {
        ...
        for (var i = sel.ranges.length - 1; i >= 0; i--) {
            var range = sel.ranges[i]
            if (range.head.ch > 100 || (i && sel.ranges[i - 1].head.line ==
                range.head.line)) { continue }
            ...
            if (mode.electricChars) {
                ...
                if (mode.electricInput.test(getLine(cm.doc, range.head.line).
                    text.
                    slice(0, range.head.ch))) {
                    indented = indentLine(cm, range.head.line, 'smart')
                }
            }
            if (indented) {
...
// convenience.
    ...
    function addEditorMethods (CodeMirror) {
        ...
        CodeMirror.prototype = {
            ...
            removeOverlay: methodOp(function (spec) {
                ...
                for (var i = 0; i < overlays.length; ++i) {
                    ...
                    if (cur == spec || typeof spec == 'string' && cur.name ==
                        spec) {
                        overlays.splice(i, 1)
                ...
                for (var i = 0; i < ranges.length; i++) {
                    ...
                    if (!range.empty()) {
                        ...
                        if (from.ch == 0 && ranges.length == newRanges.length &&
                            ...
                            replaceOneSelection(this$1.doc, i,
            ...
            getTokenTypeAt: function (pos) {
                ...
                if (ch == 0) { type = styles[2] } else {
                    for (; ;) {
                        ...
                        if ((mid ? styles[mid * 2 - 1] : 0) >=
                            ch) { after = mid } else if (styles[mid * 2 + 1] <
                            ch) { before = mid + 1 } else {
                            type = styles[mid * 2 + 2]
                            break
                        }
                    }
            ...
            getHelpers: function (pos, type) {
                ...
                for (var i$1 = 0; i$1 < help._global.length; i$1++) {
                    ...
                    if (cur.pred(mode, this$1) && indexOf(found, cur.val) ==
                        -1) { found.push(cur.val) }
                }
                return found
            },
            ...
            cursorCoords: function (start, mode) {
                ...
                if (start == null) { pos = range.head } else if (typeof start ==
                    'object') {
            ...
            lineAtHeight: function (height, mode) {
                height = fromCoordSystem(this, { top: height, left: 0 },
                    mode || 'page').top
            ...
            heightAtLine: function (line, mode, includeWidgets) {
                ...
                if (typeof line == 'number') {
                    ...
                    if (line <
                        this.doc.first) { line = this.doc.first } else if (line >
                        last) {
                        line = last
                        end = true
                    }
            ...
            addWidget: function (pos, node, scroll, vert, horiz) {
                ...
                if (vert == 'over') {
                    ...
                    } else if (pos.bottom + node.offsetHeight <=
                        vspace) { top = pos.bottom }
                ...
                if (horiz == 'right') {
                    ...
                    if (horiz == 'left') { left = 0 } else if (horiz ==
                        'middle') {
                        left = (display.sizer.clientWidth - node.offsetWidth) /
                            2
                    }
            ...
            triggerElectric: methodOp(
                function (text) { triggerElectric(this, text) }),
            ...
            moveH: methodOp(function (dir, unit) {
                ...
                this.extendSelectionsBy(function (range) {
                    if (this$1.display.shift || this$1.doc.extend ||
                        range.empty()) {
            ...
            moveV: methodOp(function (dir, unit) {
                ...
                doc.extendSelectionsBy(function (range) {
                    ...
                    if (range.goalColumn !=
                        null) { headPos.left = range.goalColumn }
                ...
                if (goals.length) {
                    for (var i = 0; i <
                    doc.sel.ranges.length; i++) { doc.sel.ranges[i].goalColumn = goals[i] }
                }
            }),
            ...
            findWordAt: function (pos) {
                ...
                if (line) {
                    ...
                    while (start > 0 &&
                    check(line.charAt(start - 1))) { --start }
            ...
            scrollIntoView: methodOp(function (range, margin) {
                if (range == null) {
                    ...
                    if (margin ==
                        null) { margin = this.options.cursorScrollMargin }
                ...
                if (range.from.line != null) {
                    ...
                    scrollToCoordsRange(this, range.from, range.to,
                        range.margin)
                }
            }),
        ...
        CodeMirror.registerHelper = function (type, name, value) {
            if (!helpers.hasOwnProperty(
                type)) { helpers[type] = CodeMirror[type] = { _global: [] } }
            helpers[type][name] = value
        }
        CodeMirror.registerGlobalHelper = function (
            type, name, predicate, value) {
...
// CONTENTEDITABLE INPUT STYLE
    ...
    ContentEditableInput.prototype.init = function (display) {
        ...
        on(div, 'paste', function (e) {
            ...
            if (ie_version <= 11) {
                setTimeout(operation(cm,
                    function () { return this$1.updateFromDOM() }), 20)
            }
        })
        ...
        on(div, 'compositionend', function (e) {
            if (this$1.composing) {
                if (e.data !=
                    this$1.composing.data) { this$1.readFromDOMSoon() }
                this$1.composing.done = true
            }
        })

        on(div, 'touchstart',
            function () { return input.forceCompositionEnd() })
    ...
    ContentEditableInput.prototype.startGracePeriod = function () {
        ...
        this.gracePeriod = setTimeout(function () {
            ...
            if (this$1.selectionChanged()) {
                this$1.cm.operation(
                    function () { return this$1.cm.curOp.selectionChanged = true })
            }
        }, 20)
    }
    ...
    ContentEditableInput.prototype.receivedFocus = function () {
        var input = this
        if (this.selectionInEditor()) { this.pollSelection() } else {
            runInOp(this.cm,
                function () { return input.cm.curOp.selectionChanged = true })
        }
    ...
    ContentEditableInput.prototype.pollSelection = function () {
        ...
        if (anchor && head) {
            runInOp(cm, function () {
                setSelection(cm.doc, simpleSelection(anchor, head),
                    sel_dontScroll)
    ...
    ContentEditableInput.prototype.pollContent = function () {
        ...
        if (to.ch == getLine(cm.doc, to.line).text.length && to.line <
            cm.lastLine()) { to = Pos(to.line + 1, 0) }
        if (from.line < display.viewFrom || to.line > display.viewTo -
            1) { return false }
        ...
        var oldText = getBetween(cm.doc, Pos(fromLine, 0),
        ...
        oldTop.charCodeAt(cutFront)) { ++cutFront }
        ...
        oldBot.charCodeAt(oldBot.length - cutEnd - 1)) { ++cutEnd }
        // Try to move start of change to start of selection if ambiguous
        if (newText.length == 1 && oldText.length == 1 && fromLine ==
            from.line) {
    ...
    ContentEditableInput.prototype.onKeyPress = function (e) {
        ...
        if (!this.cm.isReadOnly()) {
            operation(this.cm, applyTextInput)(this.cm, String.fromCharCode(
                e.charCode == null ? e.keyCode : e.charCode), 0)
        }
    }
    ...
    function domTextBetween (cm, from, to, fromLine, toLine) {
        ...
        function walk (node) {
            if (node.nodeType == 1) {
                ...
                if (markerID) {
                    var found = cm.findMarks(Pos(fromLine, 0),
                        Pos(toLine + 1, 0), recognizeMarker(+markerID))
                    if (found.length && (range = found[0].find(0))) {
                        addText(getBetween(cm.doc, range.from, range.to).
                            join(lineSep))
                    }
                    return
                }
    ...
    function locateNodeInLineView (lineView, node, offset) {
        ...
        for (var after = topNode.nextSibling, dist = textNode
            ? textNode.nodeValue.length - offset
            : 0; after; after = after.nextSibling) {
...
// TEXTAREA INPUT STYLE
    ...
    TextareaInput.prototype.init = function (display) {
        ...
        on(te, 'input', function () {
            if (ie && ie_version >= 9 &&
                this$1.hasSelection) { this$1.hasSelection = null }
            input.poll()
        })
    ...
    TextareaInput.prototype.prepareSelection = function () {
        ...
        if (cm.options.moveInputWithCursor) {
            ...
            result.teTop = Math.max(0,
                Math.min(display.wrapper.clientHeight - 10,
                    headPos.top + lineOff.top - wrapOff.top))
            result.teLeft = Math.max(0,
...
// not reset the textarea when typing, because that breaks IME).
    ...
    TextareaInput.prototype.onContextMenu = function (e) {
        var input = this, cm = input.cm, display = cm.display,
            te = input.textarea
        ...
        te.style.cssText = 'position: absolute; width: 30px; height: 30px;\n      top: ' +
            (e.clientY - wrapperBox.top - 5) + 'px; left: ' +
    ...
    function fromTextArea (textarea, options) {
        ...
        if (!options.tabindex &&
            textarea.tabIndex) { options.tabindex = textarea.tabIndex }
        if (!options.placeholder &&
            textarea.placeholder) { options.placeholder = textarea.placeholder }
        // Set autofocus to true if this textarea is focused, or if it has
        // autofocus and no other element is focused.
        if (options.autofocus == null) {
            var hasFocus = activeElt()
            options.autofocus = hasFocus == textarea ||
                textarea.getAttribute('autofocus') != null && hasFocus ==
                document.body
        }
        ...
        options.finishInit = function (cm) {
            ...
            cm.toTextArea = function () {
                ...
                if (textarea.form) {
                    ...
                    if (typeof textarea.form.submit ==
                        'function') { textarea.form.submit = realSubmit }
                }
            }
        }
...
// Set up methods on CodeMirror's prototype to redirect to the editor's document.
    ...
    for (var prop in Doc.prototype) {
        if (Doc.prototype.hasOwnProperty(prop) && indexOf(dontDelegate, prop) <
            0) {
...
// load a mode. (Preferred mechanism is the require/define calls.)
    CodeMirror.defineMode = function (name/*, mode, …*/) {
        if (!CodeMirror.defaults.mode && name !=
            'null') { CodeMirror.defaults.mode = name }
...
// Minimal default mode.
    CodeMirror.defineMode('null',
        function () { return ({ token: function (stream) { return stream.skipToEnd() } }) })
