            ...
            e.preventDefault(), $this.popover({
                ...
                on('inserted.bs.popover', function () {
                    var el = $this.data('bs.popover'),
                        tip = '3.x' === $.fn.bsVersion() ? el.tip() : $(
                            el.getTipElement())
                    tip.addClass('iconpicker-popover')
                }).
                on('shown.bs.popover', function () {
                    data.switchPage(op.icon), data.bindEvents()
                }), $this.popover('show')
        }))
    : (op.inline = !0, data.setIconset(op.iconset), $this.empty().
        append('<input type="hidden" ' + name + '></input>').
        append(op.table).
        addClass('iconpicker').
        addClass(op.align), data.switchPage(op.icon), data.bindEvents())
}
})
}, $.fn.iconpicker.Constructor = Iconpicker, $.fn.iconpicker.noConflict = function () {return $.fn.iconpicker = old, this}, $.fn.bsVersion = function () {
    return $.fn.popover.Constructor.VERSION.substr(0, 2) + 'x'
}, $(document).
    on('click', 'body', function (e) {
        $('.iconpicker').
            each(function () {
                $(this).is(e.target) || 0 !== $(this).has(e.target).length ||
                0 !== $('.popover').has(e.target).length || $(this).
                    popover('3.x' === $.fn.bsVersion() ? 'destroy' : 'dispose')
            })
